...
### 데이터 반환 방식 (DB 엔진 vs 애플리케이션)
- **조인(NLJ) 과정에서 조건에 맞는 데이터를 찾았을 때, DB 엔진과 애플리케이션(JPA 등)은 서로 다른 방식으로 데이터를 처리**한다.
- 각 레벨에서 어떤 방식으로 동작하는지 한 번 알아보자.

#### DB 레벨: 스트리밍 방식
- **DB 옵티마이저는 조인 조건이 맞는 행을 찾을 때마다, 네트워크 소켓을 통해 클라이언트(애플리케이션 서버)로 전송**하는데, 이를 **스트리밍 방식**(Streaming / Pipelining)이라고 한다.
- **조인 결과를 DB 메모리에 누적하지 않으므로, (결과 테이블의 크기와는 상관없이) DB 서버에서 OOM이 발생하지 않는다.**

#### 애플리케이션 레벨: 마치 배치처럼 동작
- DB 엔진은 스트리밍으로 데이터를 쏘고 있지만, **개발자 입장에서는 배치 방식으로 제공받는다고 느껴질 수 있는데, 이는 애플리케이션 레벨에서의 동작 방식 때문**이다. 
  - JDBC 드라이버는 DB가 소켓을 통해 전달한 데이터를 내부적으로 `List<Entity>` 컬렉션에 계속하여 추가한다.
  - 이후 DB로부터 데이터 스트림이 종료되면, 그제야 완성된 `List`를 개발자에게 반환한다.

#### 스트리밍 방식의 이점
- **페이징(`LIMIT`) 쿼리에서 탐색 중단(Short-circuit)을 할 수 있다**는 것이 스트리밍 방식이 갖는 가장 큰 이점이다.
- DB는 조건에 맞는 데이터를 찾는 즉시 클라이언트로 내보내기 때문에, 쿼리에 `LIMIT 10`이 걸려 있다면 10개의 데이터만 찾아서 흘려보내지, **나머지 데이터에 대해서는 디스크 탐색과 조인 연산을 전혀 수행하지 않는다.**

#### DB 엔진 예외: `ORDER BY`와 `GROUP BY`에 대해서는 배치처럼 동작
- **DB 엔진은 기본적으로 스트리밍 방식으로 동작하지만, `ORDER BY`와 `GROUP BY`에 대해서는 블로킹 방식으로 동작**한다.
  - **스트리밍 방식:** 개별 데이터만 봐도 판단이 가능해서, 데이터를 읽는 즉시 처리하는 방식 (대표적으로 `WHERE`, `JOIN`)
  - **블로킹 방식:** 전체 데이터를 모아야만 판단이 가능해서, 데이터를 처리하지 않고 모아두는 방식 (대표적으로 정렬, 그룹화)
- 쿼리에 `ORDER BY`나 `GROUP BY`가 포함된 경우, DB 엔진은 **전체 데이터를 조회하기 전까지는 특정 메모리 공간(Sort Buffer)이나 디스크에 타겟 데이터들을 모아 둔다.**

---

## 스레드 풀과 커넥션 풀
### 스레드 풀 (Thread Pool)
- 애플리케이션에 들어오는 요청을 병렬로 처리하기 위해 미리 생성해 둔 작업 스레드들의 집합이다.
- **요청마다 스레드를 생성하는 오버헤드를 줄이기 위해 사용**한다.
- 스프링 부트에서는 주로 **톰캣 스레드 풀**을 사용하며, **기본적으로 200개의 스레드를 제공**한다.

### 커넥션 풀 (Connection Pool)
- DB와의 물리적 연결(TCP/IP 세션 수립, 인증 등)을 미리 맺어두고 보관하는 풀이다.
- **쿼리 실행 시마다 DB와 커넥션(네트워크 연결)을 새로 맺는 오버헤드를 줄이기 위해 사용**한다.
- 스프링 부트에서는 주로 **HikariCP 커넥션 풀**을 사용하며, **기본적으로 10개의 커넥션을 제공**한다.

### 스레드 풀과 커넥션 풀의 상호작용
- 클라이언트 요청을 할당받은 톰캣 스레드는 로직을 수행하다가 **DB 접근이 필요한 시점(트랜잭션 시작, 기본적으로는 `@Transactional` 메서드 실행 시점)에 HikariCP에 커넥션을 요청**(`getConnection()`)한다.
- HikariCP에 유휴(Idle) 커넥션이 있다면 즉시 획득하여 DB 작업을 수행하고, **트랜잭션 종료 시 커넥션을 풀에 반환**한다.
- **유휴 커넥션이 없다면 스레드는 커넥션이 반환될 때까지 설정된 최대 대기 시간(`connectionTimeout`, 기본값 30초) 동안 대기**(Block) 상태에 머문다.

### 커넥션 고갈
- 일반적으로 스레드의 개수가 커넥션 수보다 많기 때문에, 대규모 트래픽이 발생하면 커넥션 고갈이 발생할 수 있다.
- **커넥션이 고갈되면 남은 스레드들은 커넥션을 얻기 위해 대기(Block) 상태**에 빠지며, 이는 **전체 서블릿 컨테이너의 응답 지연(Latency)으로 직결**된다.

### `@Transactional(propagation = Propagation.REQUIRES_NEW)`
- **현재 진행 중인 트랜잭션(부모 트랜잭션)의 존재 여부와는 무관하게, 새로운 트랜잭션(커넥션)을 시작하기 위한 전파 속성**이다.
- **현재 트랜잭션과는 무관하게 새로운 트랜잭션을 열어야 하는 경우에 사용**된다. (**예: 실패 이력 보존**)
  - 로그 이벤트를 발행해서 이벤트 메시지 큐에 던질 수 없는 상황을 생각해 보자.
  - 가령 결제 로직(부모 트랜잭션)을 수행하다가 예외가 발생하여 롤백이 되더라도, '결제 실패 로그' 자체는 남아야 한다.
  - 이런 상황에서 부모 트랜잭션에 그대로 참여하면 로그도 함께 롤백되기 때문에, 생명주기가 다른 별도의 트랜잭션을 열어야 할 것이다.
  - 이럴 때 `REQUIRES_NEW`를 사용하면 된다.

### 풀 데드락 (Pool Deadlock)
> **`REQUIRES_NEW`를 사용하면 하나의 스레드에서 2개의 커넥션을 요구하게 되기 때문에, 데드락이 발생할 여지가 있다.**

#### 예시
1. **추가 커넥션 요청:**
   - 부모 트랜잭션에서 커넥션을 획득해 사용하다가 **`REQUIRES_NEW`를 만나는 순간, 자식 트랜잭션을 시작하기 위해 새로운 커넥션을 요청**한다.
   - 참고로 스프링 부트에서는 `@Transactional` 메서드가 호출되는 즉시, 실제 DB 접근 여부와는 무관하게 커넥션이 잡힌다.
   - 이를 방지(실제 DB 접근 시점에 커넥션을 획득)하기 위해서는 `LazyConnectionDataSourceProxy`를 도입해 볼 수 있다. 
2. **데드락 발생:**
   - 가령 **트래픽이 몰려서 모든 스레드가 커넥션을 쥔 채 새로운 커넥션을 요청하는 상황이었다면, 점유와 대기(Hold and Wait)로 인해 데드락이 발생**했을 것이다. 

#### 권장 커넥션 풀 사이즈
- 이러한 풀 데드락을 방지하기 위해 HikariCP 공식 문서에서는 커넥션 풀 사이즈를 다음과 같이 권장하고 있다.
  - **`커넥션 풀 사이즈 = max(톰캣 스레드 수 * (스레드당 최대 동시 요구 커넥션 수 - 1) + 1, 10)`**
...
- 수정자 주입이나 필드 주입은 생성자 호출 이후에 값을 할당하는 방식으모, `final` 키워드를 사용할 수 없다.
...
- 분기 처리를 통해 빈을 고를 필요 없이, `Map`으로 모든 빈을 주입받아 두고, 넘겨받은 식별자(빈 이름)을 키로 사용하여 맵에서 즉시 해당 객체를 꺼내 실행하면 된다.

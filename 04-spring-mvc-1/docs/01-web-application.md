...
- **JSP(Java Server Pages) 파일 역시 실행 시점에 서블릿 코드로 변환**되어 서블릿 컨테이너 내부에서 동작한다.
...
### 요청마다 쓰레드를 생성하는 방식
#### 동작 방식 및 장점
- **고객의 요청이 들어올 때마다 새로운 쓰레드를 생성하여 로직을 처리하는 방식**이다.
...
- 여기서 알 수 있듯, 서블릿은 자바 객체이기 때문에 **동적으로 웹 페이지를 생성하려고 하더라도 결국은 자바 코드 내부에 HTML 문자열을 작성**해야 하는 문제가 존재한다. 
  ```java
    ...
    out.println("</html>");
  ``` 
- 웹 페이지의 대부분을 차지하는 마크업(HTML 문자열)을 모두 자바 코드로 처리해야 하므로, 화면(UI) 개발 생산성이 크게 떨어졌다.

#### JSP (Java Server Pages)
- 자바 코드 내부에 HTML을 작성해야 하는 서블릿의 단점을 극복하기 위해, **역으로 기본 틀을 HTML로 작성하고 필요한 부분에만 자바 코드를 삽입하는 방식**인 JSP가 등장했다.
- 덕분에 UI 개발 편의성은 크게 향상되었지만, **여전히 화면을 렌더링하는 코드(HTML)와 비즈니스 로직(Java)이 하나의 파일에 혼재하는 문제**가 존재했다. 
- 이로 인해 복잡한 페이지의 경우, 파일 하나가 수천 줄을 넘어가서 유지보수가 사실상 불가능했다.

### MVC 패턴의 등장과 프레임워크로의 진화
#### MVC 패턴 도입 (관심사의 분리)
- 하나의 파일(JSP)에서 모든 것을 처리하던 방식을 버리고, **Model-View-Controller로 역할을 분리**했다.
  - **Controller (서블릿/자바):** 비즈니스 로직을 처리하고, 클라이언트 요청을 제어한다.
  - **View (JSP):** Controller가 전달한 데이터를 바탕으로 화면 렌더링만 전담한다.
...
- 개발자가 비즈니스 로직에만 집중할 수 있도록 **서블릿이 네트워크 및 프로토콜 레벨의 복잡한 작업(TCP/IP 연결, HTTP 메시지 파싱 등)을 자동화해 준 것은 맞지만, 애플리케이션 레벨에서는 여전히 비효율적인 반복 작업이 남아있던 것**이다.
  ...
  - ...
- 이러한 **보일러플레이트 코드(여러 곳에서 재사용되는 반복적인 코드)를 자동화**하는 것에 더해, **편리한 웹 기능(데이터 바인딩, 검증, 예외 처리 등)도 지원하는 여러 MVC 프레임워크가 등장**하게 되었다.
...
#### 기존 배포 방식 (`.war`)
- 과거에는 **서버 장비에 Tomcat(WAS)을 먼저 설치한 뒤, 개발한 애플리케이션 코드를 `.war` 파일로 빌드하고 Tomcat 내부의 특정 폴더에 밀어 넣어 배포**했다.
...
> **스프링 웹 기술은 크게 Spring MVC(동기/블로킹)와 Spring WebFlux(비동기/논블로킹)로 나뉜다.**

#### Spring WebFlux의 특징
- **Netty라는 비동기 네트워크 프레임워크를 기반으로 동작**한다.
- **함수형 스타일**로 개발하며, **Caller의 제어권을 즉시 반환(Non-blocking)하므로 쓰레드가 대기 상태에 빠지지 않는다.**
- 위임한 작업이 끝날 때까지 쓰레드가 대기하는 구조가 아니기 때문에, 쓰레드가 많이 필요 없게 되고, 이는 **컨텍스트 스위칭 비용의 절감**으로 이어진다.

#### Spring WebFlux의 한계점
- **기술적 난이도:**
  - 비동기 함수형 코드를 작성하고 디버깅하는 난이도가 높다.
- **RDB 지원의 한계:**
  - **자바에서 RDB와 통신하기 위한 표준 API인 JDBC는 동기/블로킹 방식**이다.
  - 즉, **RDB 통신을 위해서는 쿼리를 날리고 응답이 올 때까지 쓰레드가 대기**해야 하는데, WebFlux에서는 이러한 **쓰레드 블로킹이 DB 병목을 야기**하게 된다. (보유한 쓰레드 수 자체가 적기 때문)
- 이러한 이유에서 **대부분의 서버에서는 아직 Spring MVC를 주력으로 사용**하고 있다.

### 뷰 템플릿 엔진의 역사
> **백엔드 서버에서 자바 데이터를 조합해 HTML을 동적으로 렌더링하기 위해 사용하는 뷰 템플릿 엔진의 역사**를 살펴보자.

#### JSP (Java Server Pages)
- 초기 웹 개발의 표준이었으나, **실행 속도가 상대적으로 느리고 기능 확장에 한계**가 있다.
- 속도와 확장성 이외에도 다음과 같은 한계점들이 존재한다.
  - `<% out.println(name); %>`와 같이 자바 코드와 HTML 마크업이 혼재되기 때문에 **파일(시스템)의 유지보수가 어려워진다.**
  - 웹 서버를 거치지 않고 브라우저에서 JSP 파일을 직접 열면, 자바 코드가 그대로 노출되며 화면 디자인이 깨지기 때문에 **프론트엔드 개발자와의 협업이 불편**하다.

#### Freemarker / Velocity
- JSP의 단점(성능, 문법 등)을 개선하여 한때 널리 사용되었다.
- 하지만 점차 **기술 발전과 업데이트가 정체**되었고, 최신 스프링 프레임워크의 발전 속도를 따라가지 못해 현재는 사용 비중이 크게 줄어들었다.

#### 타임리프 (Thymeleaf)
> **현재 스프링 진영에서 공식적으로 권장하는 뷰 템플릿 엔진**이다.

- **내츄럴 템플릿 (Natural Template):**
  - **순수 HTML 마크업 구조를 파괴하지 않고, `<div th:text="${name}">임시 이름</div>`처럼 태그의 속성(Attribute)을 통해 동적 변수를 주입**한다.
  - 웹 서버를 거치지 않고 브라우저에서 직접 HTML 파일을 열더라도, **문법 오류 없이 HTML 마크업이 정상적으로 렌더링**되므로, 프론트엔드 개발자와의 협업이 불편하지 않다.
- **스프링 MVC와의 강력한 통합:**
  - 스프링의 **데이터 바인딩, 폼(Form) 처리, 검증(Validation) 및 에러 메시지 출력 기능 등을 공식적으로 지원**한다.